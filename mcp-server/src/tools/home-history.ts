...
export const tools: ToolDef[] = [
  defineTool({
    name: 'home_history',
    description:
      "Version history for this agent home. `status` reports on/off, where it's kept, the last snapshot, and, for a synced home, the local folder setup will use; its only write puts back a .git link a synced folder deleted (`restored`). `setup` turns it on (in place, or kept outside a cloud-synced folder with its path and sandbox grants recorded in .claude/settings.local.json) and makes the first snapshot; idempotent. A home whose version history is set up some other way is left alone. Runs outside the Bash sandbox.",
    inputSchema: {
    ...
    handler: async ({ action, name, startOver }) => {
      if (action === 'setup') {
        return { ...setupHistory(FOLDERS.HOME, { name, startOver }), codexWired: codexWired() };
      }

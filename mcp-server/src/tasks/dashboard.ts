...
import { scanAllTasks, scanMalformedTasks } from './scan';
...
export const formatDashboard = (
  ...
  const header = `> Tasks hub — auto-generated by ${agentDisplayName()} from task frontmatter on every mutation. Do not edit this file directly: tasks change via the task tools, goals via the weekly-goals / monthly-goals / yearly-goals skills.\n> Last updated: ${todayDate()}\n`;

  // Unparseable files are absent from every section below, so without this
  // banner a broken task reads as a deleted one.
  const malformedBlock = malformed.length
    ? `\n> [!warning] ${malformed.length} task file${malformed.length === 1 ? '' : 's'} could not be parsed and ${malformed.length === 1 ? 'is' : 'are'} missing from the sections below. Frontmatter must open with \`---\` on line 1.\n${malformed.map((path) => `> - \`${basename(path)}\``).join('\n')}\n`
    : '';
...
export const writeDashboard = (): DashboardCounts => {
  ...
  writeFileSync(tasksFile, formatDashboard(sections, goalsBlock, scanMalformedTasks()));

...
- **Codex CLI is a second host.** A checkout installs as a Codex plugin
  ...
  marketplace (`.agents/plugins/marketplace.json`, named `agentdev-kevin` like Claude's).
  ...
  entry plus a `SessionEnd` capture) and a `[mcp_servers.kevin]` table in
  ...
  (Step 7c) and `upgrade` run it; the operator trusts the folder and the two hook entries
  ...
  Codex plugin versions, are not exercised yet (lo-066 Phase 3).

...
- Const objects over enums: `const Status = { Open: 'open', Done: 'done' } as const` with `type Status = (typeof Status)[keyof typeof Status]`. `as const` goes on the object itself, not on arrays derived from it.
...
- Dispatch on a discriminant with a `switch`, each case in its own block (`case 'ready': { … }`), not an `if` chain.

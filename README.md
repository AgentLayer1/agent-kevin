...
|------|-----------------|--------|
| **Claude Code** or **Codex** | The host Kevin plugs into; one home works from either, or both | [docs.claude.com](https://docs.claude.com/en/docs/claude-code/setup) · [developers.openai.com/codex](https://developers.openai.com/codex) |
...
The plugin ships its own developer marketplace (`.claude-plugin/marketplace.json`, named `agentdev-kevin`), so a local clone of `agent-kevin` is itself a marketplace you can register. Edits to the clone are live after `/reload-plugins`; consumers use Option A instead.
...
### Option C: Install under OpenAI Codex

The same checkout is a Codex plugin (`.codex-plugin/plugin.json`, carrying the skills) and ships a developer marketplace (`.agents/plugins/marketplace.json`, named `agentdev-kevin`, the Codex counterpart of Option B) so a clone is installable as-is. Consumers register the `agentlayer` marketplace repo instead (`codex plugin marketplace add AgentLayer1/agentlayer-agent-marketplace`, then `codex plugin add agent-kevin@agentlayer`). From a clone, register it once, install, then run `init` from the home:

```bash
git clone https://github.com/AgentLayer1/agent-kevin ~/Developer/agent-kevin
cd ~/Developer/agent-kevin/mcp-server && bun install
codex plugin marketplace add ~/Developer/agent-kevin
codex plugin add agent-kevin@agentdev-kevin

cd ~/Documents/Agents/Kevin   # a home that already exists; see the next paragraph
codex
```

For a home that already exists (the supported path in 0.4.1), run `$upgrade` there, or `/agent-kevin:upgrade` from Claude Code, which asks whether you also run Codex from the home and writes the same wiring. A brand-new home created with `$init` under Codex is not exercised yet; create it from Claude Code for now. Codex reads the home's `AGENTS.md` natively; the identity stack, the indexes, and the session capture arrive through per-home hooks that `init` writes to `.codex/hooks.json` (trusted once via `/hooks`), and Kevin's MCP server is registered per home in `.codex/config.toml` (Codex reads it once you trust the folder). A home works from both CLIs, and a session ended in one is in the other's context next time. Codex installs a *copy* of the checkout into its plugin cache, so after pulling new code run `codex plugin remove agent-kevin@agentdev-kevin && codex plugin add agent-kevin@agentdev-kevin`, then `$upgrade` from the home.
...
Under Codex the same `bin/kevin` entry points run from per-home hooks (`.codex/hooks.json`, generated by `init` and `upgrade`, trusted once via `/hooks`): one `SessionStart` entry delivers the static context (registered with `additionalContextLimit: 0`, lifting Codex's default cap of about 2,500 tokens per hook), `SessionEnd` plus `PreCompact` entries capture the session, the same two moments Claude Code captures at, and a `PreToolUse` entry runs the same cwd-drift guard (Codex presents shell commands to hooks as `Bash`, the same contract). The MCP server is registered per home too (`.codex/config.toml`), because a plugin-bundled server on Codex 0.153 cannot learn which home it serves. Session blocks record which harness and model produced them. The same generator carries the home's posture over from its Claude settings: a `[permissions.kevin]` profile denies the model every read of the runtime secrets store and of any `.env` (the MCP server still reads them), keeps `.git` writable so commits stay routine, and lists the code path and any `additionalDirectories` as workspace roots; every `Bash(…)` pattern in `permissions.ask` becomes a `prefix_rule` in `.codex/rules/kevin.rules` that prompts you, with `approvals_reviewer = "user"` so a human answers, as under Claude Code (the reviewer model is opt-in: on this plugin's tests it denied named pushes and local commits). The user level gets a paste-ready note in `.kevin/updates/codex-user-config.md`: the keys Codex accepts only in `~/.codex/config.toml` (telemetry, analytics, feedback, terminal, update checks, the approval policy and reviewer) and, while that file lacks them, a permission profile built from your Claude user-level `Read(…)` denies and a `~/.codex/rules/default.rules` built from its ask list, none of it naming an agent; the plugin never writes a user-level file, and until you paste, the home's own profile carries the user-level denies too. Limits under Codex today: skills that name `${CLAUDE_PLUGIN_ROOT}` resolve it through the plugin-root line in the SessionStart header (an instruction, not a shell variable), the session radar reads Claude transcripts only, and the profile enables the network inside the sandbox, since an approved escalation does not lift a profile, so the outbound gate is the rules file alone; domain allowlists need Codex's experimental network proxy.
...
agent-kevin/
├── .claude-plugin/
│   ├── plugin.json          # Claude Code plugin manifest: skills/, hooks/claude.json, the kevin MCP server
│   └── marketplace.json     # developer marketplace (agentdev-kevin) so a clone installs under Claude Code
├── .codex-plugin/
│   └── plugin.json          # Codex plugin manifest: the same skills/ (hooks + MCP server are registered per home)
├── .agents/plugins/
│   └── marketplace.json     # the same developer marketplace for Codex
...
```bash
export PATH="$HOME/Developer/agent-kevin/bin:$PATH"   # your clone; a marketplace install lives under ~/.claude/plugins/cache/agentlayer/agent-kevin/<version>
...
A: The marketplace trust prompt was missed. Recover inside Claude Code with `/plugin marketplace add github:AgentLayer1/agentlayer-agent-marketplace` followed by `/plugin install agent-kevin@agentlayer` (a local clone: add the clone's path, then `agent-kevin@agentdev-kevin`), then `/exit` and relaunch.

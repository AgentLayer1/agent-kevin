...
Auth is a fine-grained, **read-only** personal access token in `<HOME>/.kevin/secrets/.env` as `GITHUB_TOKEN` (gh honors it and skips the keychain). The easiest setup is the **GitHub pack**: `/agent-kevin:configure-skills` → tick GitHub (or tick it at `/agent-kevin:init` Step 8). It grants the ten tool permissions, ensures `.kevin/secrets/.env` exists, and walks you through minting the PAT — resource owner set to the repos' owner, repository permissions all read-only (**Pull requests · Issues · Metadata · Actions · Contents**). You paste the value into your editor, never into chat. The read-only token is a second wall behind the read-only tool surface.

...
Under Codex the same `bin/kevin` entry points run from per-home hooks (`.codex/hooks.json`, generated by `init` and `upgrade`, trusted once via `/hooks`): one `SessionStart` entry delivers the static context (registered with `additionalContextLimit: 0`, lifting Codex's default cap of about 2,500 tokens per hook), and one `SessionEnd` entry captures the session. The MCP server is registered per home too (`.codex/config.toml`), because a plugin-bundled server on Codex 0.153 cannot learn which home it serves. Session blocks record which harness and model produced them.

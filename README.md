...
The same checkout is a Codex plugin (`.codex-plugin/plugin.json`, carrying the skills) and ships a developer marketplace (`.agents/plugins/marketplace.json`, named `agentdev-kevin`, the Codex counterpart of Option B) so a clone is installable as-is. Register it once, install, then run `init` from the home:
...
Under Codex the same `bin/kevin` entry points run from per-home hooks (`.codex/hooks.json`, generated by `init` and `upgrade`, trusted once via `/hooks`): one `SessionStart` entry delivers the static context (registered with `additionalContextLimit: 0`, lifting Codex's default cap of about 2,500 tokens per hook), and `SessionEnd` plus `PreCompact` entries capture the session, the same two moments Claude Code captures at. The MCP server is registered per home too (`.codex/config.toml`), because a plugin-bundled server on Codex 0.153 cannot learn which home it serves. Session blocks record which harness and model produced them.
...
├── .agents/plugins/
│   └── marketplace.json     # developer marketplace (agentdev-kevin) so a clone installs under Codex

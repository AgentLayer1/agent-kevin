...
- Build the smallest script that does or proves the job, never a framework ([laziness protocol](laziness-protocol.md)). A scratch lever gets a `mktemp` path. Commit it when the work outlives the session.

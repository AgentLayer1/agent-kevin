...
2. Name the data shape and its organizing structure before writing any logic ([model the domain](../principles/model-the-domain.md), [foundational thinking](../principles/foundational-thinking.md)). If the work crosses a function boundary, sketch it first ([design](../design.md)). If the right shape isn't obvious, design it twice ([exhaust the design space](../principles/exhaust-the-design-space.md)).
3. Write the throughput checkpoint as four todo items. A dimension that doesn't apply keeps its line as `n/a: <reason>`.
   ...
   - **Shared mutable state:** split the target first ([separate before serializing](../principles/separate-before-serializing.md)). Serialize only for a real invariant.
   - **Smallest safe decomposition:** if one worker is best, say why.
4. Subtract first. Delete what the new shape makes dead before building on it.

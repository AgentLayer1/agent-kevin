...
Read the JSON into the `💾 Brain` output line. `CLEAN` and the `SKIPPED_*` statuses are one neutral clause, never dressed up as problems. `NOT_A_REPO` means history is off (or its folder is gone, which its `detail` says), and `SKIPPED_NO_SNAPSHOT` means its setup stopped before the first snapshot (sync never makes that one): either way one neutral clause pointing at `/agent-kevin:history`, which finishes it. `SKIPPED_LINK_MISSING` means a synced folder deleted the home's one-line `.git` link: one clause saying the next session start or `/agent-kevin:history` restores it. `SKIPPED_OTHER_HOME` means this folder's link points at another folder's history (a copied home): surface its `detail` and never commit around it. `COMMIT_BLOCKED` means git couldn't write the repo — surface its `detail`, which carries the likely fix (history kept outside the folder needs its grants, which the history skill records); don't hand-run git to force the commit through. The closing interview's own task mutations (step 12) land in the next sync's commit — that's fine.

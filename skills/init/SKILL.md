...
For `<CODEX_HOOKS_ROW>`: if Step 7c ran → `✅ Codex wiring  .codex/hooks.json (4 entries; trust them via /hooks) + .codex/config.toml (kevin MCP server, permission profile) + .codex/rules/kevin.rules (<n> prompt rules) · user-level keys: .kevin/updates/codex-user-config.md`; otherwise omit the row.

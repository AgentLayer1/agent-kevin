...
> - ☐ GitHub — read-only PR + GitHub Actions access (`github_pr_*`, `github_run_*`) so Kevin can review PRs, walk their own, and brief a second model on them or on any branch (the `pr-review`, `pr-walkthrough`, and `adversarial-review` skills) and diagnose failing CI builds

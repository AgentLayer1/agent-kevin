...
1. `Grep` `<HOME>/reports/reviews/` for `pr-<n>`, `#<n>`, the branch name, and each changed file's basename. A prior report on the same PR means this run **supersedes** it: say so in the summary, carry forward its still-open items, and list what changed since. A `pr-<n>-walkthrough` report (from `pr-walkthrough`) is not superseded; it is the author's own stated intent and test plan, and the review checks the diff against it. An `adversarial-*` dossier (from `pr-adversarial`) on the same change is carried the same way: its Ledger lists what a second model already raised and how each finding was dispositioned.
